...
Memory silence is the bug this entry exists to kill. A real SE-agent inception output was called out for never mentioning memory at all — the `state_shape` axis classified the agent and then the classification was dropped, producing a design that read as if memory weren't a dimension. That's the gap. The fix is procedural:
...
So my read for the pipeline hook: **`state_shape` is necessary but not sufficient.** The classifier should keep emitting it (it's the right prior and it cleanly handles working + episodic), but the pipeline needs **one additional binary signal — `learns_from_experience` (does the agent improve over time?)** — to resolve procedural memory, and it should treat **semantic-memory need as a domain-grounding question asked independently of state shape** (effectively a `grounds_in_corpus` signal, which the workload classifier may already imply via tool/RAG presence). With those two signals added alongside `state_shape`, memory need falls out deterministically and the dedicated decision step becomes a lookup against the class table rather than a judgment call. Without them, procedural and semantic memory remain the two kinds most likely to be dropped — which is precisely the silence this entry is closing.
