"""MCP server exposing the v0.7 discovery agent + the intake (priors)
...
async def tool_submit_customer_turn(
    ...
    """Submit a customer turn. Runs the v0.7 pipeline (extractors → mega-
    agent with tools) and returns the agent's response + state summary."""
    client = _llm_client()
    session, mega = _get_or_rehydrate_session(session_id)
    turn, checklist = await run_v07_turn(client, session, mega, message)
    return {
        "ok": True,
...
def _render_spec_markdown(session: DiscoverySession) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by discovery-inception v0.7 (lazy synthesis + deterministic close-out).*")
    return "\n".join(lines)
